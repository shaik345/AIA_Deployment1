...
<xsl:stylesheet version="2.0" xmlns:ebo="http://xmlns.oracle.com/EnterpriseObjects/Core/EBO/Item/V2" xmlns:eboebo="http://xmlns.oracle.com/EnterpriseObjects/Core/EBO/Item/V2" xmlns:aia="http://www.oracle.com/XSL/Transform/java/oracle.apps.aia.core.xpath.AIAFunctions" xmlns:corecom="http://xmlns.oracle.com/EnterpriseObjects/Core/Common/V2" xmlns:xacml-context="urn:oasis:names:tc:xacml:2.0:context:schema:cd:04" xmlns:xref="http://www.oracle.com/XSL/Transform/java/oracle.tip.xref.xpath.XRefXPathFunctions" xmlns:xp20="http://www.oracle.com/XSL/Transform/java/oracle.tip.pc.services.functions.Xpath20" xmlns:bpws="http://schemas.xmlsoap.org/ws/2003/03/business-process/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ora="http://schemas.oracle.com/xpath/extension" xmlns:ehdr="http://www.oracle.com/XSL/Transform/java/oracle.tip.esb.server.headers.ESBHeaderFunctions" xmlns:orcl="http://www.oracle.com/XSL/Transform/java/oracle.tip.pc.services.functions.ExtFunc" xmlns:ids="http://xmlns.oracle.com/bpel/services/IdentityService/xpath" xmlns:hwf="http://xmlns.oracle.com/bpel/workflow/xpath" xmlns:wsa="http://schemas.xmlsoap.org/ws/2003/03/addressing" xmlns:str="http://www.oracle.com/XSL/Transform/java/java.lang.String" xmlns:ns1="http://www.oracle.com/communications/pricing" exclude-result-prefixes="xsl xacml-context xref xp20 bpws ora ehdr orcl ids hwf aia wsa corecom coresalesorder sblorderdata sblasi str">
   <xsl:param name="EscapedCommitProductRespMessage_Sponsorship"/>
   <xsl:variable name="CommitProductRespMessage" select="orcl:parseEscapedXML($EscapedCommitProductRespMessage_Sponsorship)"/>
   ...
   <xsl:template match="/">
      <xsl:apply-templates select="eboebo:SyncItemListEBM"/>
   </xsl:template>
   <xsl:template match="eboebo:SyncItemListEBM">
      <eboebo:SyncItemListResponseEBM>
         <xsl:for-each select="$CommitProductRespMessage/ns1:pricingResult[ns1:type = 'DISTRIBUTION_OFFERING']">
            <xsl:variable name="PDCExternalID" select="ns1:externalId"/>
            <xsl:variable name="PDCID" select="ns1:pdcID"/>
            <xsl:variable name="EBMExternalCommonId" select="/eboebo:SyncItemListEBM/eboebo:DataArea[eboebo:SyncItemList/corecom:ItemIdentification/corecom:AlternateObjectKey/corecom:ID = $PDCExternalID]/eboebo:SyncItemList/corecom:ItemIdentification/corecom:BusinessComponentID"/>
            ...
            <xsl:variable name="IfPDCIdExists" select="xref:lookupXRef('oramds:/apps/AIAMetaData/xref/ITEM_ITEMID.xref',$XREFSourceCol,$EBMExternalCommonId,$XREFTargetCol,false())"/>
               ...
               <xsl:if test="boolean($IfExternalIdExists = '' and $IfPDCIdExists = '')">
                  <xsl:choose>
                     <xsl:when test="count(xref:lookupPopulatedColumns('oramds:/apps/AIAMetaData/xref/ITEM_ITEMID.xref',$XREFSourceCol,$EBMExternalCommonId,false())/column)=0">
                        <xsl:value-of select="xref:populateXRefRow('oramds:/apps/AIAMetaData/xref/ITEM_ITEMID.xref',$XREFSourceCol,$EBMExternalCommonId,$XREFTargetCol,ns1:pdcID,'ADD')"/>
                     ...
                     <xsl:otherwise>
                        <xsl:value-of select="xref:populateXRefRow('oramds:/apps/AIAMetaData/xref/ITEM_ITEMID.xref',$XREFSourceCol,$EBMExternalCommonId,$XREFTargetCol,ns1:pdcID,'LINK')"/>
         ...
         <xsl:for-each select="$CommitProductRespMessage/ns1:pricingResult[ns1:type = 'DISTRIBUTION_RATE_PLAN']">
            <xsl:variable name="PDCID" select="ns1:pdcID"/>
            <xsl:variable name="PDCSponsorshipModelName" select="ns1:name"/>
            <xsl:variable name="IfSponsorshipModelIdExists" select="xref:lookupXRef('oramds:/apps/AIAMetaData/xref/SPONSORSHIPOBJECT_ID.xref',$XREFSourceCol,$PDCSponsorshipModelName,$XREFTargetCol,false())"/>
            <xsl:variable name="prodID">
               <xsl:if test="boolean($IfSponsorshipModelIdExists = '')">
                  <xsl:choose>
                     <xsl:when test="count(xref:lookupPopulatedColumns('oramds:/apps/AIAMetaData/xref/SPONSORSHIPOBJECT_ID.xref',$XREFSourceCol,$PDCSponsorshipModelName,false())/column)=0">
                        <xsl:value-of select="xref:populateXRefRow('oramds:/apps/AIAMetaData/xref/SPONSORSHIPOBJECT_ID.xref',$XREFSourceCol,$PDCSponsorshipModelName,$XREFTargetCol,ns1:pdcID,'ADD')"/>
                     </xsl:when>
                     <xsl:otherwise>
                        <xsl:value-of select="xref:populateXRefRow('oramds:/apps/AIAMetaData/xref/SPONSORSHIPOBJECT_ID.xref',$XREFSourceCol,$PDCSponsorshipModelName,$XREFTargetCol,ns1:pdcID,'LINK')"/>
                     </xsl:otherwise>
                  </xsl:choose>
               </xsl:if>
            </xsl:variable>
